...
class ChangeRateScenarios extends Page implements HasTable
{
    ...
    {
        return (string) PrognosisChangeRate::query()->count();
    }
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                TextColumn::make('type')
                    ...
                    ->formatStateUsing(function (string $state): string {
                        $p = PrognosisType::where('code', $state)->first();

                        return $p->label ?? ucfirst($state);
                    })
                    ->icon(fn (string $state) => optional(PrognosisType::where('code', $state)->first())->icon)
                    ->color(fn (string $state) => optional(PrognosisType::where('code', $state)->first())->color ?? 'gray')
                    ->sortable()
                    ->searchable(),
                ...
                TextColumn::make('updated_by')
                    ...
                    ->formatStateUsing(fn ($state, $record) => optional(PrognosisType::where('code', $record->type)->first()?->updatedBy)->name ?? '—')
                    ->badge()
                    ->color('gray'),
            ...
            ->recordUrl(function ($record) {
                return route('filament.system.pages.change-rate-assets', [
