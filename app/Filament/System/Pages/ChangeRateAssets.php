...
class ChangeRateAssets extends Page implements HasTable
{
    ...
    {
        $scenarioLabel = PrognosisType::where('code', $this->scenario)->value('label') ?? ucfirst($this->scenario);
        ...
        return $table
            ...
            ->columns([
                TextColumn::make('type')
                    ...
                    ->formatStateUsing(function (string $state): string {
                        return AssetType::query()->where('type', $state)->value('name') ?? ucfirst($state);
                    })
                    ->color('primary')
                    ->sortable()
                    ->searchable(),
            ...
            ->filters([
                SelectFilter::make('asset_type')
                    ->label('Asset Type')
                    ->options(PrognosisChangeRate::assetTypeOptions())
                    ->multiple()
                    ->preload()
            ...
            ->recordUrl(function ($record) {
                return route('filament.system.pages.change-rate-table', [
    ...
    public function getAssetTypes(): array
    {
        ...
        foreach ($assetTypes as $type) {
            ...
            $assets[] = [
                'type' => $type,
                'label' => AssetType::query()->where('type', $type)->value('name') ?? ucfirst($type),
