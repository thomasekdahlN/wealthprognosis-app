...
class TaxRealizationTest extends TestCase
{
    ...
    public function test_it_calculates_tax_realization_for_company_group()
    {
        ...
        $result = $taxRealization->taxCalculationRealization(false, false, 'company', 'stock', 2022, 50000, 10000, 0, 0);
        $repo = new \App\Services\Tax\TaxConfigRepository('no');
        $expectedRate = $repo->getTaxRealizationRate('stock', 2022);
        $this->assertEquals($expectedRate, $result->taxRate);
    ...
    {
        $taxRealization = new TaxRealizationService('no');
        $result = $taxRealization->taxCalculationRealization(false, false, 'individual', 'stock', 2022, 50000, 10000, 0, 0);
        $repo = new \App\Services\Tax\TaxConfigRepository('no');
        $expectedRate = $repo->getTaxRealizationRate('stock', 2022);
        $this->assertEquals($expectedRate, $result->taxRate);
        $this->assertEquals(round((50000 - 10000) * $expectedRate), $result->taxAmount);
    }

    public function test_it_calculates_tax_realization_for_unknown_group()
    {
        $taxRealization = new TaxRealizationService('no');
        $result = $taxRealization->taxCalculationRealization(false, false, 'unknown', 'stock', 2022, 50000, 10000, 0, 0);
        $repo = new \App\Services\Tax\TaxConfigRepository('no');
        $expectedRate = $repo->getTaxRealizationRate('stock', 2022);
        $this->assertEquals($expectedRate, $result->taxRate);
        $this->assertEquals(round((50000 - 10000) * $expectedRate), $result->taxAmount);
    ...
    {
        $taxRealization = new TaxRealizationService('no');
        $result = $taxRealization->taxCalculationRealization(false, false, 'company', 'stock', 2022, 0, 10000, 0, 0);
        $repo = new \App\Services\Tax\TaxConfigRepository('no');
        $expectedRate = $repo->getTaxRealizationRate('stock', 2022);
        $this->assertEquals($expectedRate, $result->taxRate);
        $this->assertEquals(0, $result->taxAmount);
