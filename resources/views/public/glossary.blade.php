    ...
    @php
        ...
        foreach ($groups as $group) {
            foreach ((array) ($group['terms'] ?? []) as $term) {
                $definedTerms[] = [
                    '@type' => 'DefinedTerm',
                    'name' => trim(html_entity_decode(strip_tags((string) ($term['term'] ?? '')))),
                    'description' => trim(html_entity_decode(strip_tags((string) ($term['def'] ?? '')))),
                    'inDefinedTermSet' => route('glossary').'#glossary',
                ];
            }
        }
    @endphp
        ...
        '@graph' => [
            ...
            [
                ...
                'itemListElement' => [
                    ...
                    ['@type' => 'ListItem', 'position' => 2, 'name' => $page->get('hero.badge'), 'item' => route('glossary')],
                ],
            ],
        ],
...
@section('content')
    ...
    <section class="relative hero-gradient overflow-hidden" aria-labelledby="glossary-title">
